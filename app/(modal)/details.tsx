...
import { getRestaurantById, restaurants } from '@/assets/data/restaurant';
...
const Details = ({}) => {
  ...
  // if restaurant[] only contains 1 restaurant
  ...
  return (
    <>
      <ParallaxScrollView
        ...
        renderBackground={() =>  ( <Image source={restaurant?.img} style={{ height: 300, width: '100%' }} />) } // Example using first restaurant image
        ...
        <View style={styles.detailsContainer}>
          ...
          <SectionList
            ...
            keyExtractor={(restaurant, index) => `${restaurant?.id + index}`}
            scrollEnabled={false}
            sections={DATA}
      ...
      <Animated.View style={[styles.stickySegments, animatedStyles]}>
        <View style={styles.segmentsShadow}>
          <ScrollView ref={scrollRef} horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.segmentScrollview}>
            {restaurant?.food.map((restaurant, index) => (
              <TouchableOpacity
                ref={(ref) => (itemsRef.current[index] = ref!)}
                key={index}
                style={activeIndex === index ? styles.segmentButtonActive : styles.segmentButton}
                onPress={() => selectCategory(index)}>
                <Text style={activeIndex === index ? styles.segmentTextActive : styles.segmentText}>{restaurant.category}</Text>
              </TouchableOpacity>
            ))}
          </ScrollView>
        </View>
